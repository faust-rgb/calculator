...
std::function<Scalar(const std::vector<Scalar>&)> make_scalar_bound_func(
    ...
    if (num_vars == 1) {
        const std::string& v0 = vars[0];
        return [eval, v0](const std::vector<Scalar>& pt) -> Scalar {
            return eval({{v0, pt[0]}});
        };
    } else if (num_vars == 2) {
        const std::string& v0 = vars[0];
        const std::string& v1 = vars[1];
        return [eval, v0, v1](const std::vector<Scalar>& pt) -> Scalar {
            return eval({{v0, pt[0]}, {v1, pt[1]}});
        };
    }
    throw std::runtime_error("make_scalar_bound_func: unsupported variable count");
}
...
Scalar line_integral(const IntegrationContext& ctx, const std::string& expr,
    ...
    auto integrand = [&](const std::vector<Scalar>& pt) {
        ...
        const Scalar h = integration_engine::adaptive_derivative_step(t);
        Scalar t_ld = (t);
        Scalar t_plus_h = (t + h);
...
Scalar surface_integral(const IntegrationContext& ctx, const std::string& expr,
    ...
    auto integrand = [&](const std::vector<Scalar>& pt) {
        ...
        const Scalar hu = integration_engine::adaptive_derivative_step(u);
        const Scalar hv = integration_engine::adaptive_derivative_step(v);
