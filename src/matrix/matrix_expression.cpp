...
Matrix divisors_vector(long long value) {
    ...
    for (long long d = 1; d * d <= n; ++d) {
        ...
        small.push_back(static_cast<long double>(d));
        if (d != n / d) {
...
private:
    ...
    long double compare_values(const Value& lhs, const Value& rhs, const std::string& op) {
        ...
        if (op == ">") return l > r ? 1.0L : 0.0L;
        if (op == ">=") return l >= r ? 1.0L : 0.0L;
        if (op == "==") return mymath::is_near_zero(l - r, 1e-10) ? 1.0L : 0.0L;
        if (op == "!=") return !mymath::is_near_zero(l - r, 1e-10) ? 1.0L : 0.0L;
        return 0.0L;
    }
    ...
    Matrix parse_matrix_literal() {
        ...
        Matrix result(rows.size(), max_cols, 0.0L);
    ...
    Value parse_matrix_function(const std::string& name) {
        ...
        if (name == "extended_gcd" || name == "xgcd") {
            ...
            const long long gcd =
                extended_gcd_local(parse_integer_argument(arguments[0], *scalar_evaluator_, "extended_gcd"),
                                   ...
                                   &x,
                                   &y);
            Matrix result(1, 3, 0.0L);
            result.at(0, 0) = static_cast<long double>(gcd);
            result.at(0, 1) = static_cast<long double>(x);
            result.at(0, 2) = static_cast<long double>(y);
            return Value::from_matrix(result);
        }
        ...
        if (name == "abs") {
            ...
            Value v;
            if (try_evaluate_expression(arguments[0], *scalar_evaluator_, *matrix_lookup_, *complex_lookup_, matrix_functions_, native_functions_, &v)) {
                ComplexNumber z;
        ...
        if (native_functions_) {
            ...
            if (it != native_functions_->end()) {
                ...
                sv_args.reserve(arguments.size());
                for (const auto& arg : arguments) {
                    sv_args.push_back(evaluate_to_stored_value(arg));
                }
                StoredValue result = it->second(sv_args);
                Value v;
                if (result.is_matrix && result.matrix_ptr) {
                    v = Value::from_matrix(*result.matrix_ptr);
                } else if (result.is_complex) {
                    v = Value::from_complex(result.complex);
                } else {
                    v = Value::from_scalar(result.decimal);
                }
                return v;
            }
        }
    ...
    StoredValue evaluate_to_stored_value(const std::string& expression) const {
        Value v = evaluate_value_argument(expression);
        StoredValue sv;
        if (v.is_matrix) {
            sv.is_matrix = true;
            sv.matrix_ptr = std::make_shared<Matrix>(v.matrix);
        } else if (v.is_complex) {
            sv.is_complex = true;
            sv.complex = v.complex;
        } else {
            sv.decimal = v.scalar;
        }
        return sv;
...
bool try_evaluate_expression(const std::string& expression,
                             ...
                             Value* value) {
    // Unified AST is the canonical syntax path. The legacy matrix parser below
    // remains only as a compatibility backend for domain-specific functions
    // that have not yet been registered in FunctionRegistry.
    if (auto ast = compile_expression_ast(expression)) {
        ...
        context.set_external_variable_lookup([&](const std::string& name, StoredValue* out) {
            Matrix matrix_value;
            if (matrix_lookup && matrix_lookup(name, &matrix_value)) { *out = StoredValue(matrix_value); return true; }
            ComplexNumber complex_value;
            if (complex_lookup && complex_lookup(name, &complex_value)) { *out = StoredValue(complex_value); return true; }
            try { *out = StoredValue(scalar_evaluator(name)); return true; } catch (...) { return false; }
        });
        if (native_functions) {
    ...
    try {
        MatrixExpressionParser parser(trimmed, &scalar_evaluator, &matrix_lookup, &complex_lookup, matrix_functions, native_functions);
        *value = parser.parse();
        return true;
    } catch (...) {
        return false;
