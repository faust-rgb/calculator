...
std::string build_help_topic(const std::string& topic) {
    ...
    if (topic == "examples") {
        ...
        "  pdf_normal(x, m, s) Normal distribution PDF\n"
        "  cdf_normal(x, m, s) Normal distribution CDF\n"
        "  poisson_pmf(k, l)   Poisson distribution PMF\n"
        "  binom_pmf(n, k, p)  Binomial distribution PMF\n"
        "  pdf_t(x, df)        Student's t-distribution PDF\n"
        "  cdf_chi2(x, df)     Chi-squared distribution CDF\n"
        "  t_test(mu0, data)   One-sample t-test p-value\n"
        "  chi2_test(obs, exp) Chi-squared goodness-of-fit test\n"
        "  spearman(x, y)      Spearman rank correlation\n"
        "  describe(data)      Professional statistical summary\n"
        ...
        "  Constants:          pi, e, c, G, h, k, NA, inf, oo\n"
    ...
    if (topic == "matrix") {
        return
        "Matrix guide:\n"
        "  Create:  [a,b;c,d] vec mat zeros randmat random_matrix eye identity\n"
        ...
        "  Signal:  dft fft idft ifft conv convolve hann hanning hamming blackman\n"
        ...
        "  Example: randmat(2, 3) or randmat(2, 3, -1, 1)\n"
        "  Example: transpose(m)\n"
        "  Example: inverse(m)\n"
    ...
    if (topic == "symbolic") {
        ...
        "  Field ops:      divergence([Fx, Fy, Fz], x, y, z), curl([Fx, Fy, Fz], x, y, z)\n"
        "  2D curl:        curl_2d([Fx, Fy], x, y) returns scalar ∂Fy/∂x - ∂Fx/∂y\n"
        "  Laplacian:      laplacian(expr, x, y, z)\n"
        "  Advanced diff:  implicit_diff(F, y, x), param_deriv(x(t), y(t), t), directional(expr, x, y, vx, vy)\n"
        ...
        "  Aliases:        inverse_laplace, inverse_fourier, z_transform, inverse_z\n"
        "  Z-transform:    ztrans(expr, n, z), iztrans(expr, z, n)\n"
        "  Line integrals: line_integral(f_or_F, [x(t), y(t)], t, a, b)\n"
        "  Surface integrals: surface_integral(f_or_F, [x, y, z], u, a, b, v, c, d)\n"
        "  Vector theorems: greens_theorem([Fx, Fy], [x(t), y(t)], t, a, b), stokes_theorem([Fx, Fy, Fz], [x, y, z], u, a, b, v, c, d)\n"
        "  Divergence theorem: divergence_theorem([Fx, Fy, Fz], [x, y, z], u, a, b, v, c, d)\n"
    ...
    if (topic == "analysis") {
        ...
        "  Limits:         limit(expr, x0), limit(expr, x0, direction), limit(expr, x, x0)\n"
    ...
    if (topic == "functions") {
        ...
        "  Numeric:       abs sign floor ceil round trunc min max clamp sum mean avg median mode percentile quartile var std sample_var sample_std iqr mad skewness kurtosis factorial nCr binom nPr bernoulli fib is_prime next_prime prev_prime prime_pi euler_phi mobius rand randn randint\n"
        "  Legacy nums:   factorial nCr nPr\n"
        "  Distributions: pdf_normal cdf_normal pdf_t cdf_t pdf_chi2 cdf_chi2 pdf_f cdf_f pdf_exp cdf_exp poisson_pmf poisson_cdf binom_pmf binom_cdf\n"
        "  Statistics:    mean median mode var std sample_var sample_std iqr mad skewness kurtosis cov correlation spearman weighted_mean t_test t_test2 chi2_test describe\n"
        "  Signals:       step delta heaviside impulse fourier ifourier laplace ilaplace ztrans iztrans\n"
        "  Discrete sig:  dft fft idft ifft conv convolve hann hamming blackman\n"
        ...
        "  Integer:       gcd lcm mod factor prev_prime prime_pi euler_phi mobius divisors egcd extended_gcd xgcd\n"
        "  Base convert:  bin oct hex base\n"
        "  Aggregate:     sum mean avg median mode percentile quartile var std skewness kurtosis cov corr\n"
        ...
        "  Vector calc:   gradient jacobian hessian divergence curl curl_2d laplacian directional\n"
        "  Integrals:     line_integral surface_integral\n"
        "  Theorems:      greens_theorem stokes_theorem divergence_theorem\n"
