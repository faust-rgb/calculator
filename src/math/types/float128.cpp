...
#include <iomanip>
#include <climits>
#include <cfloat>  // For LDBL_MANT_DIG

namespace mymath {

namespace {

// Float128 precision configuration
// The DLD (Double-Long-Double) technique provides approximately 36-38 significant digits
// We use dynamic thresholds based on the actual precision of long double on the platform

// Estimate the effective precision of long double (in decimal digits)
// LDBL_MANT_DIG is the number of base-B digits (B = FLT_RADIX, usually 2)
// For binary floating point: decimal digits ≈ mantissa_bits * log10(2)
// 64-bit mantissa (x86 extended precision) ≈ 19.2 decimal digits
// 113-bit mantissa (IEEE quad) ≈ 34 decimal digits
inline int estimate_long_double_precision() {
    return static_cast<int>(LDBL_MANT_DIG * 0.30102999566398119521373889472449L);
}

// Dynamic epsilon based on platform precision
// For DLD, we get roughly 2x the precision of a single long double
inline long double compute_float128_epsilon() {
    int prec = estimate_long_double_precision();
    long double eps = 1.0L;
    for (int i = 0; i < prec * 2 - 2; ++i) eps *= 0.1L;
    return eps;
}

// Threshold for detecting trigonometric singularities (near π/2, 3π/2, etc.)
// This should be larger than epsilon but small enough to catch singularities
inline long double compute_trig_singular_threshold() {
    int prec = estimate_long_double_precision();
    long double thresh = 1.0L;
    for (int i = 0; i < prec / 2 - 1; ++i) thresh *= 0.1L;
    return thresh;
}

// Float128 relative epsilon (computed at startup)
const long double kFloat128Epsilon = compute_float128_epsilon();
const long double kFloat128TrigSingular = compute_trig_singular_threshold();

// Maximum iterations for series expansions - adaptive based on expected convergence
// For Taylor series, convergence rate depends on argument magnitude
constexpr int kMaxSeriesIterations = 1000;  // Upper bound safety limit
constexpr int kDefaultSeriesIterations = 100;  // Default for well-conditioned inputs
...
float128_t hypot(float128_t a, float128_t b) {
    float128_t abs_a = float128::abs(a);
    float128_t abs_b = float128::abs(b);
    if (abs_a.hi == 0) return abs_b;
    if (abs_b.hi == 0) return abs_a;
    if (abs_a > abs_b) {
        float128_t ratio = abs_b / abs_a;
        return abs_a * float128::sqrt(float128_t(1.0L) + ratio * ratio);
    }
    float128_t ratio = abs_a / abs_b;
    return abs_b * float128::sqrt(float128_t(1.0L) + ratio * ratio);
}

float128_t normalize_angle(float128_t x) {
    float128_t two_pi_val = float128::two_pi();
    float128_t result = float128::fmod(x, two_pi_val);
    float128_t p = float128::pi();
    if (result > p) {
...
float128_t sqrt(float128_t a) {
    if (float128::isnan(a)) return {std::numeric_limits<long double>::quiet_NaN(), std::numeric_limits<long double>::quiet_NaN()};
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        float128_t diff = float128::abs(next_res - res);
        if (diff.hi < kFloat128Epsilon * float128::abs(next_res).hi) {
            break;
        }
        res = next_res;
    }
...
float128_t exp_high_prec(float128_t a) {
    ...
    if (a.hi < -11400.0L) return {0.0L, 0.0L};
    ...
    float128_t k_f = float128::round(a * inv_ln2);
    int k = static_cast<int>(k_f.hi);
    float128_t r = a - k_f * ln2;

    // Adaptive iteration count based on |r|
    // Smaller |r| means faster convergence
    long double abs_r = float128::abs(r).hi;
    int max_iter = kMaxSeriesIterations;
    if (abs_r < 0.5L) max_iter = 60;
    if (abs_r < 0.25L) max_iter = 40;
    if (abs_r < 0.1L) max_iter = 30;
    ...
    for (int i = 1; i <= max_iter; ++i) {
        ...
        accum.add(term);
        if (float128::abs(term).hi < kFloat128Epsilon * float128::abs(accum.get()).hi) break;
    }

    return float128::ldexp(accum.get(), k);
}
...
// Extended precision pi for Payne-Hanek style reduction
float128_t reduce_trig_large(float128_t a) {
    // If |a| is relatively small, remainder is sufficient
    if (float128::abs(a).hi < 1e12L) {
        return float128::remainder(a, float128::two_pi());
    }
    float128_t two_pi_val = float128::two_pi();
    float128_t a_reduced = a;
    while (float128::abs(a_reduced).hi > 1e12L) {
        float128_t scale(1e10L);
        float128_t scaled_pi = two_pi_val * scale;
        float128_t q = float128::trunc(a_reduced / scaled_pi);
        a_reduced = a_reduced - scaled_pi * q;
    }
    return float128::remainder(a_reduced, two_pi_val);
}

float128_t sin(float128_t a) {
    if (float128::isinf(a) || float128::isnan(a)) return {std::numeric_limits<long double>::quiet_NaN(), std::numeric_limits<long double>::quiet_NaN()};

    float128_t x = reduce_trig_large(a);

    float128_t half_pi = float128::half_pi();
    ...
    // Adaptive iteration count based on |x|
    long double abs_x = float128::abs(x).hi;
    int max_iter = kDefaultSeriesIterations;
    if (abs_x < 0.5L) max_iter = 50;
    if (abs_x < 0.25L) max_iter = 35;
    if (abs_x < 0.1L) max_iter = 25;
...
float128_t tan(float128_t a) {
    ...
    if (float128::abs(c).hi < kFloat128TrigSingular) {
        throw std::domain_error("tan is undefined at odd multiples of pi/2");
    }
    return float128::sin(a) / c;
}

float128_t asin(float128_t a) {
    if (float128::abs(a).hi > 1.0L) return {std::numeric_limits<long double>::quiet_NaN(), std::numeric_limits<long double>::quiet_NaN()};
    if (a.hi == 1.0L && a.lo >= 0) return float128::pi() * 0.5L;
    if (a.hi == -1.0L && a.lo <= 0) return float128::pi() * -0.5L;

    float128_t one_minus_x2 = float128_t(1.0L) - a * a;
    return float128::atan(a / float128::sqrt(one_minus_x2));
}

float128_t acos(float128_t a) {
    if (float128::abs(a).hi > 1.0L) return {std::numeric_limits<long double>::quiet_NaN(), std::numeric_limits<long double>::quiet_NaN()};
    return float128::pi() * 0.5L - float128::asin(a);
}

float128_t sec(float128_t a) {
    return float128_t(1.0L) / float128::cos(a);
}

float128_t csc(float128_t a) {
    return float128_t(1.0L) / float128::sin(a);
}

float128_t cot(float128_t a) {
    return float128::cos(a) / float128::sin(a);
}

float128_t asec(float128_t a) {
    if (float128::abs(a).hi < 1.0L) {
        return {std::numeric_limits<long double>::quiet_NaN(), std::numeric_limits<long double>::quiet_NaN()};
    }
    return float128::acos(float128_t(1.0L) / a);
}
...
float128_t acsc(float128_t a) {
    ...
    return float128::asin(float128_t(1.0L) / a);
}

float128_t acot(float128_t a) {
    return float128::pi() * 0.5L - float128::atan(a);
}
...
float128_t tanh(float128_t a) {
    ...
    float128_t e2x = float128::exp(a * 2.0L);
    return (e2x - float128_t(1.0L)) / (e2x + float128_t(1.0L));
}

float128_t asinh(float128_t a) {
    return float128::ln(a + float128::sqrt(a * a + float128_t(1.0L)));
}
...
float128_t acosh(float128_t a) {
    ...
    return float128::ln(a + float128::sqrt(a - float128_t(1.0L)) * float128::sqrt(a + float128_t(1.0L)));
}
...
float128_t atanh(float128_t a) {
    ...
    return float128::ln((float128_t(1.0L) + a) / (float128_t(1.0L) - a)) * 0.5L;
}

float128_t log10(float128_t a) {
    static const float128_t ln10 = from_string("2.302585092994045684017991454684364207601");
    return float128::ln(a) / ln10;
}

float128_t log2(float128_t a) {
    static const float128_t ln2 = from_string("0.6931471805599453094172321214581765680755");
    return float128::ln(a) / ln2;
}

float128_t log1p(float128_t a) {
    if (float128::abs(a).hi < 0.125L) {
        // For small |a|, use Taylor series for log(1+a) = a - a²/2 + a³/3 - ...
        // Adaptive iteration count based on |a|
        long double abs_a = float128::abs(a).hi;
        int max_iter = kDefaultSeriesIterations;
        if (abs_a < 0.0625L) max_iter = 50;
        if (abs_a < 0.03125L) max_iter = 35;
        if (abs_a < 0.01L) max_iter = 25;
    ...
    return float128::ln(float128_t(1.0L) + a);
}
...
float128_t pow(float128_t base, float128_t exponent) {
    ...
    if (base.hi < 0) {
        float128_t exp_rounded = float128::round(exponent);
        if (float128::abs(exponent - exp_rounded).hi < kFloat128Epsilon) {
            long double exp_int = exp_rounded.hi;
            float128_t abs_base = float128::abs(base);
            float128_t result = float128::exp(exp_rounded * float128::ln(abs_base));
    ...
    return float128::exp(exponent * float128::ln(base));
}
...
float128_t atan(float128_t a) {
    ...
    float128_t res(x);
    for (int i = 0; i < 6; ++i) {
        float128_t ty = float128::tan(res);
        float128_t next_res = res + (a - ty) / (float128_t(1.0L) + ty * ty);
        float128_t diff = float128::abs(next_res - res);
        if (diff.hi < kFloat128Epsilon * (float128::abs(next_res).hi + 1.0L)) {
            break;
        }
        res = next_res;
    }
...
float128_t atan2(float128_t y, float128_t x) {
    if (x.hi > 0) return float128::atan(y / x);
    if (x.hi < 0) {
        if (y.hi >= 0) return float128::atan(y / x) + float128::pi();
        return float128::atan(y / x) - float128::pi();
    }
    if (y.hi > 0) return float128::pi() * 0.5L;
    if (y.hi < 0) return float128::pi() * -0.5L;
    return {std::numeric_limits<long double>::quiet_NaN(), std::numeric_limits<long double>::quiet_NaN()};
}
...
float128_t exp2(float128_t a) {
    static const float128_t ln2 = from_string("0.6931471805599453094172321214581765680755");
    return float128::exp(a * ln2);
}

float128_t expm1(float128_t a) {
    if (float128::abs(a).hi < 0.125L) {
        // For small |a|, use Taylor series for exp(a)-1 = a + a²/2! + a³/3! + ...
        // Adaptive iteration count based on |a|
        long double abs_a = float128::abs(a).hi;
        int max_iter = kDefaultSeriesIterations;
        if (abs_a < 0.0625L) max_iter = 50;
    ...
    return float128::exp(a) - float128_t(1.0L);
}
...
int ilogb(float128_t a) {
    ...
    if (float128::isnan(a)) return FP_ILOGBNAN;
    if (float128::isinf(a)) return INT_MAX;
...
float128_t logb(float128_t a) {
    if (a.hi == 0) return {std::numeric_limits<long double>::quiet_NaN(), std::numeric_limits<long double>::quiet_NaN()};
    if (float128::isnan(a)) return a;
    if (float128::isinf(a)) return {std::numeric_limits<long double>::infinity(), 0.0L};
    return float128_t(std::logb(a.hi));
}

float128_t nextafter(float128_t a, float128_t b) {
    if (float128::isnan(a) || float128::isnan(b)) return {std::numeric_limits<long double>::quiet_NaN(), std::numeric_limits<long double>::quiet_NaN()};
    if (a == b) return b;
...
float128_t fmax(float128_t a, float128_t b) {
    if (float128::isnan(a)) return b;
    if (float128::isnan(b)) return a;
    return (a > b) ? a : b;
}
...
std::string to_string(float128_t a, int precision) {
    if (float128::isnan(a)) return "nan";
    if (float128::isinf(a)) return a.hi > 0 ? "inf" : "-inf";
    ...
    if (a.hi >= 1e18L || (a.hi > 0 && a.hi < 1e-10L)) {
        // Calculate exponent: find e such that 1 <= a * 10^(-e) < 10
        int exponent = 0;
        float128_t temp = a;
        ...
        while (temp.hi > 0 && temp.hi < 1.0L) {
            temp = temp * 10.0L;
            exponent--;
        }

        // Now temp is in [1, 10), output as mantissa
        float128_t integral = float128::floor(temp);
        float128_t fractional = temp - integral;

        s += std::to_string(static_cast<int>(integral.hi));

        if (precision > 0) {
            s += ".";
            for (int i = 0; i < precision; ++i) {
                fractional = fractional * 10.0L;
                float128_t digit_f = float128::floor(fractional);
        ...
        s += "e" + std::to_string(exponent);
        return s;
    }

    float128_t integral = float128::floor(a);
...
float128_t from_string(const std::string& s) {
    ...
    if (i < s.size() && (s[i] == 'e' || s[i] == 'E')) {
        ...
        if (exp_val != 0) {
            result = result * float128::pow(float128_t(10.0L), float128_t(exp_val * exp_sign));
        }
    }
