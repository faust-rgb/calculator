...
float128_t sqrt(float128_t a) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (diff.hi < kFloat128Epsilon * precise128::abs(next_res).hi) {
            break;
        }
        res = next_res;
    }
...
float128_t exp_high_prec(float128_t a) {
    ...
    long double abs_r = precise128::abs(r).hi;
    ...
    for (int i = 1; i <= max_iter; ++i) {
        ...
        accum.add(term);
        if (precise128::abs(term).hi < kFloat128Epsilon * precise128::abs(accum.get()).hi) break;
    }
...
float128_t reduce_trig_large(float128_t a) {
    ...
    if (precise128::abs(a).hi < 1e12L) {
        return precise128::remainder(a, precise128::two_pi());
    }
    float128_t two_pi_val = precise128::two_pi();
    float128_t a_reduced = a;
    while (precise128::abs(a_reduced).hi > 1e12L) {
        float128_t scale(1e10L);
        float128_t scaled_pi = two_pi_val * scale;
        float128_t q = precise128::trunc(a_reduced / scaled_pi);
        a_reduced = a_reduced - scaled_pi * q;
    }
    return precise128::remainder(a_reduced, two_pi_val);
}
...
float128_t sin(float128_t a) {
    ...
    long double abs_x = precise128::abs(x).hi;
...
float128_t tan(float128_t a) {
    ...
    if (precise128::abs(c).hi < kFloat128TrigSingular) {
...
float128_t log1p(float128_t a) {
    if (precise128::abs(a).hi < 0.125L) {
        ...
        long double abs_a = precise128::abs(a).hi;
        int max_iter = kDefaultSeriesIterations;
...
float128_t pow(float128_t base, float128_t exponent) {
    ...
    if (base.hi < 0) {
        ...
        if (precise128::abs(exponent - exp_rounded).hi < kFloat128Epsilon) {
        ...
        for (int den = 1; den <= 100; ++den) {
            ...
            if (precise128::abs(scaled - num_f).hi < kFloat128Epsilon * 1000.0L) {
...
float128_t atan(float128_t a) {
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        float128_t diff = precise128::abs(next_res - res);
        if (diff.hi < kFloat128Epsilon * (precise128::abs(next_res).hi + 1.0L)) {
            break;
        }
        res = next_res;
    }
...
std::string to_string(float128_t a, int precision) {
    ...
    if (a.hi >= 1e18L || (a.hi > 0 && a.hi < 1e-10L)) {
        ...
        float128_t integral = precise128::floor(temp);
