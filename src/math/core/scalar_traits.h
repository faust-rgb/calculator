...
// ============================================================================
// Floating Point Traits & Predicates
// ============================================================================

template <typename T = Scalar>
inline bool scalar_isnan(T x) {
    if constexpr (std::is_same_v<T, float128_t>) return float128::isnan(x);
    else if constexpr (std::is_same_v<T, PreciseDecimal>) return x.is_nan_value();
    else return false;
}

template <typename T = Scalar>
inline bool scalar_isinf(T x) {
    if constexpr (std::is_same_v<T, float128_t>) return float128::isinf(x);
    else if constexpr (std::is_same_v<T, PreciseDecimal>) return x.is_infinity();
    else return false;
}

template <typename T = Scalar>
inline bool scalar_isfinite(T x) {
    if constexpr (std::is_same_v<T, float128_t>) return float128::isfinite(x);
    else if constexpr (std::is_same_v<T, PreciseDecimal>) return !x.is_infinity() && !x.is_nan_value();
    else return true;
}

template <typename T = Scalar>
inline bool scalar_is_near_zero(T x, T eps) {
    if constexpr (std::is_same_v<T, float128_t>) return float128::is_near_zero(x, eps);
    else return scalar_abs(x) <= eps;
}
