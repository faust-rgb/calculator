...
std::string format_number(Scalar value) {
    ...
    if (mymath::is_integer(value, precision::sqrt_epsilon<Scalar>())) {
    ...
    if (mymath::approximate_fraction(value, &n, &d, 999, precision::sqrt_epsilon<Scalar>())) {
    ...
    if (symbolic.find_first_not_of("-0123456789.") != std::string::npos) {
        ...
        if (!pi_form || mymath::abs(value - mymath::scalar_pi<Scalar>()) < precision::sqrt_epsilon<Scalar>()) {
            return symbolic;
        }
    }
