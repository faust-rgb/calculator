...
SymbolicExpression make_sorted_sum(std::vector<SymbolicExpression> terms) {
    ...
    return has_combined ? combined : SymbolicExpression::number(0.0L);
}
...
SymbolicExpression build_polynomial_expression_from_coefficients(
    ...
    return has_term ? result : SymbolicExpression::number(0.0L);
}
...
bool symbolic_decompose_linear(const SymbolicExpression& expression,
    ...
    if (symbolic_polynomial_coefficients_from_simplified(expression.simplify(),
                                                        ...
                                                        &coefficients) &&
        coefficients.size() <= 2) {
        *b = coefficients.size() > 0 ? coefficients[0] : SymbolicExpression::number(0.0L);
        *a = coefficients.size() > 1 ? coefficients[1] : SymbolicExpression::number(0.0L);
        return true;
    }
    return false;
}
