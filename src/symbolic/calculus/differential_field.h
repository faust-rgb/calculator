...
public:
    ...
    Scalar to_double() const {
        return Scalar(static_cast<long long>(numerator)) / Scalar(static_cast<long long>(denominator));
    }
...
// Note: Use ExactRational directly to avoid conflict with math/precise/rational.h
