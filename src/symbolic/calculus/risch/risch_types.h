...
struct ComplexRoot {
    ...
    static ComplexRoot real(const SymbolicExpression& r) {
        return {r, SymbolicExpression::number(Scalar(0.0L)), false, false};
    }
...
struct RischIntegrationResult {
    ...
    static RischIntegrationResult non_elementary(const std::string& msg = "") {
        return {false, IntegralType::kNonElementary, SymbolicExpression::number(Scalar(0.0L)), msg, SpecialFunction::kEi};
    }
    static RischIntegrationResult proof_failed(const std::string& msg = "") {
        return {false, IntegralType::kProofFailed, SymbolicExpression::number(Scalar(0.0L)), msg, SpecialFunction::kEi};
    }
    ...
    static RischIntegrationResult unknown(const std::string& msg = "") {
        return {false, IntegralType::kUnknown, SymbolicExpression::number(Scalar(0.0L)), msg, SpecialFunction::kEi};
    }
...
struct RDEResult {
    ...
    static RDEResult no_solution(const std::string& reason) {
        return {RDEResultType::kNoSolution, SymbolicExpression::number(Scalar(0.0L)), reason};
    }

    static RDEResult cannot_prove(const std::string& reason) {
        return {RDEResultType::kCannotProve, SymbolicExpression::number(Scalar(0.0L)), reason};
    }
...
struct AlgebraicNumberRepresentation {
    ...
    static AlgebraicNumberRepresentation integer(int n) {
        std::vector<SymbolicExpression> coeffs = {
            SymbolicExpression::number(Scalar(static_cast<long long>(-(n)))),
    ...
    static AlgebraicNumberRepresentation rational(int p, int q) {
        std::vector<SymbolicExpression> coeffs = {
            SymbolicExpression::number(Scalar(-(p)) / Scalar(q)),
            SymbolicExpression::number(Scalar(1.0L))
        };
        AlgebraicNumberRepresentation num;
        num.minimal_polynomial = SymbolicPolynomial(coeffs, "_t");
...
struct AlgebraicExtensionInfo {
    ...
    static AlgebraicExtensionInfo nth_root(
        ...
        // 最小多项式: t^n - u = 0
        std::vector<SymbolicExpression> coeffs(n + 1, SymbolicExpression::number(Scalar(0.0L)));
        coeffs[0] = symbolic_expression_internal::make_negate(u).simplify();  // -u
        coeffs[n] = SymbolicExpression::number(Scalar(1.0L));  // t^n
        ...
        SymbolicExpression t_n_minus_1 = symbolic_expression_internal::make_power(t, SymbolicExpression::number(Scalar(static_cast<long long>(n - 1))));
        ext.derivation = (u_deriv / (SymbolicExpression::number(Scalar(static_cast<long long>(n))) * t_n_minus_1)).simplify();
...
struct QuotientRingElement {
    ...
    static QuotientRingElement zero(const std::string& t_var, int degree) {
        return QuotientRingElement{
            std::vector<SymbolicExpression>(degree, SymbolicExpression::number(Scalar(0.0L))),
            t_var,
            degree
        };
    }
    ...
    static QuotientRingElement constant(const SymbolicExpression& c, const std::string& t_var, int degree) {
        std::vector<SymbolicExpression> coeffs(degree, SymbolicExpression::number(Scalar(0.0L)));
        coeffs[0] = c;
    ...
    static QuotientRingElement from_polynomial(const SymbolicPolynomial& poly,
        ...
        std::vector<SymbolicExpression> coeffs(n, SymbolicExpression::number(Scalar(0.0L)));
        ...
        if (poly.divide(modulus, &q, &r)) {
            ...
            // 除法失败，手动归约
            std::vector<SymbolicExpression> temp_coeffs(poly.degree() + 1, SymbolicExpression::number(Scalar(0.0L)));
            for (int i = 0; i <= poly.degree(); ++i) {
                temp_coeffs[i] = poly.coefficient(i);
            }

            // 提取模多项式系数
            std::vector<SymbolicExpression> mod_coeffs(n, SymbolicExpression::number(Scalar(0.0L)));
            for (int k = 0; k < n; ++k) {
        ...
        for (int i = 0; i < static_cast<int>(coefficients.size()); ++i) {
            if (!SymbolicPolynomial::coeff_is_zero(coefficients[i])) {
                if (i == 0) {
                    ...
                    result = (result + coefficients[i] * symbolic_expression_internal::make_power(t, SymbolicExpression::number(Scalar(static_cast<long long>(i))))).simplify();
                }
            }
        }
        return result;
    ...
    QuotientRingElement multiply(const QuotientRingElement& other,
        ...
        std::vector<SymbolicExpression> prod_coeffs(prod_degree, SymbolicExpression::number(Scalar(0.0L)));
        ...
        std::vector<SymbolicExpression> mod_coeffs(modulus_degree, SymbolicExpression::number(Scalar(0.0L)));
        ...
        while (changed && iteration < max_iterations) {
            ...
            for (int i = prod_degree - 1; i >= modulus_degree; --i) {
                ...
                prod_coeffs[i] = SymbolicExpression::number(Scalar(0.0L));
    ...
    bool inverse(const SymbolicPolynomial& modulus, QuotientRingElement* result) const {
        ...
        // 将 s 的所有系数除以 gcd
        std::vector<SymbolicExpression> inv_coeffs(modulus_degree, SymbolicExpression::number(Scalar(0.0L)));
...
private:
    ...
    static bool extended_gcd(const SymbolicPolynomial& a, const SymbolicPolynomial& b,
        ...
        if (b.degree() < 0) {
            *g = a;
            *s = SymbolicPolynomial({SymbolicExpression::number(Scalar(1.0L))}, a.variable_name());
            *t = SymbolicPolynomial({SymbolicExpression::number(Scalar(0.0L))}, a.variable_name());
            return true;
        }
...
public:
    ...
    QuotientRingElement derivative(const std::string& x_var,
        ...
        for (int i = 0; i < modulus_degree; ++i) {
            ...
            if (i > 0) {
                SymbolicExpression term = (SymbolicExpression::number(Scalar(static_cast<long long>(i))) *
