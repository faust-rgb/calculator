...
Solution SymbolicSolver::solve_system_groebner(
    ...
    for (const auto& b : basis) {
        ...
        Scalar val;
        if (simplified.is_number(&val) && !mymath::is_near_zero(val, precision::sqrt_epsilon<Scalar>())) {
...
Solution SymbolicSolver::solve_quadratic(
    ...
    if (a.is_number(&a_ld)) {
        Scalar a_val(a_ld);
        if (mymath::abs(a_val) < precision::epsilon<Scalar>()) {
            // 退化为线性方程
    ...
    if (discriminant.is_number(&disc_ld)) {
        ...
        if (mymath::abs(disc_val) < precision::epsilon<Scalar>()) {
            // 重根: x = -b/(2a)
...
Solution SymbolicSolver::solve_cubic(
    ...
    if (all_numeric) {
        ...
        if (!mymath::is_near_zero(a_val, precision::epsilon<Scalar>())) {
            // 规范化为 x^3 + px + q = 0
            ...
            if (mymath::is_near_zero(delta, precision::epsilon<Scalar>())) {
                // 重根情况
                if (mymath::is_near_zero(p, precision::epsilon<Scalar>()) && mymath::is_near_zero(q, precision::epsilon<Scalar>())) {
                    // 三重根 x = -b/(3a)
