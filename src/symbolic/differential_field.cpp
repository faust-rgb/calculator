...
ExactRational ExactRational::from_double(long double value, int64_t max_den) {
...
DifferentialField DifferentialTowerBuilder::build(
    ...
    for (const auto& [arg, kind] : extensions) {
        ...
        switch (kind) {
            ...
            default:
                ext.derivation = SymbolicExpression::number(0.0L);
        }
...
IndependenceCheck DifferentialTowerBuilder::check_independence(
    ...
    if (kind == DifferentialExtension::Kind::kLogarithmic) {
        ...
        if (normalized_arg.node_->type == NodeType::kMultiply) {
            ...
            // 检查每个因子是否在塔中
            SymbolicExpression substitution_sum = SymbolicExpression::number(0.0L);
        ...
        for (const auto& ext : current_field.tower) {
            if (ext.kind == DifferentialExtension::Kind::kAlgebraic) {
                ...
                if (ratio.is_number(&ratio_val) && mymath::abs(ratio_val) > 1e-12) {
                    ...
                    // 如果 ratio 是完全平方，可以简化
                    long double sqrt_ratio = mymath::sqrt(ratio_val);
