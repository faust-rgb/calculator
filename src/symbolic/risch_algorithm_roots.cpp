...
std::pair<int, int> RischAlgorithm::compute_laurent_degree_bounds(
    ...
    if (ext.kind == DifferentialExtension::Kind::kExponential) {
        ...
        if (symbolic_polynomial_coefficients_from_simplified(g.simplify(), ext.t_name, &g_coeffs)) {
            upper = static_cast<int>(g_coeffs.size()) - 1;
        } else {
            upper = 10;
        }
        ...
        double m_val = 0.0;
        if (ratio.is_number(&m_val)) {
            int m = static_cast<int>(std::round(m_val));
            if (std::abs(m_val - m) < 1e-9 && m > upper) {
                upper = m;
            }
        }
    } else {
        upper = compute_rde_degree_bound(SymbolicPolynomial::from_expression(f, ext.t_name),
                                        SymbolicPolynomial::from_expression(g, ext.t_name),
                                        tower, tower_index);
    }
...
int RischAlgorithm::compute_laurent_valuation(
    ...
    if (symbolic_polynomial_coefficients_from_simplified(g.simplify(), ext.t_name, &g_coeffs)) {
        for (std::size_t i = 0; i < g_coeffs.size(); ++i) {
            if (!SymbolicPolynomial::coeff_is_zero(g_coeffs[i])) {
                val_g = static_cast<int>(i);
                break;
            }
        }
    }
    ...
    if (ratio.is_number(&n_val)) {
        ...
        if (std::abs(n_val - n) < 1e-9 && n < val_g) {
            return n;
        }
    }
...
bool RischAlgorithm::solve_laurent_rde(const SymbolicExpression& f,
    ...
    solution->has_polynomial_part = false;
    solution->has_logarithmic_part = false;
    solution->has_special_part = false;

    if (tower_index < 0) {
        // 有理情况，使用标准 RDE 求解
        IntegrationResult res = solve_rde(f, g, x_var, tower, tower_index);
        if (res.success && res.type == IntegralType::kElementary) {
            solution->has_polynomial_part = true;
            std::vector<SymbolicExpression> coeffs;
            if (symbolic_polynomial_coefficients_from_simplified(res.value, x_var, &coeffs)) {
                solution->polynomial_part = SymbolicPolynomial(coeffs, x_var);
            }
            return true;
        }
        return false;
    ...
    // 扩展系数数组以包含负次幂
    int total_size = positive_bound - negative_bound + 1;
    std::vector<SymbolicExpression> y_coeffs(total_size, SymbolicExpression::number(0.0));

    SymbolicExpression u_prime = (ext.derivation / t).simplify();

    // 从高次到低次求解 (处理消去)
    for (int i = positive_bound; i >= negative_bound; --i) {
        int idx = i - negative_bound;

        SymbolicExpression g_i = (i >= 0 && i < (int)g_coeffs.size()) ?
                                  g_coeffs[i] : SymbolicExpression::number(0.0);

        // y = sum c_i t^i
        // y' + fy = sum (c_i' + i*u'*c_i + f*c_i) t^i = sum g_i t^i
        // 因此：c_i' + (f + i*u') c_i = g_i
        
        SymbolicExpression f_i = (f + SymbolicExpression::number(static_cast<double>(i)) * u_prime).simplify();

        IntegrationResult c_res = solve_rde(f_i, g_i, x_var, tower, tower_index - 1);
        if (c_res.success && c_res.type == IntegralType::kElementary) {
            y_coeffs[idx] = c_res.value;
        } else if (!expr_is_zero(g_i)) {
            // 如果无法解出系数且 g_i 非零，则可能无解
            return false;
        }
    }

    // 构建结果
    SymbolicExpression result_expr = SymbolicExpression::number(0.0);
    bool has_nonzero = false;

    for (int i = positive_bound; i >= negative_bound; --i) {
        int idx = i - negative_bound;
        if (!SymbolicPolynomial::coeff_is_zero(y_coeffs[idx])) {
            has_nonzero = true;
            SymbolicExpression term = (y_coeffs[idx] *
                make_power(t, SymbolicExpression::number(i))).simplify();
            result_expr = (result_expr + term).simplify();
        }
    }

    if (has_nonzero) {
        solution->has_polynomial_part = true;
        std::vector<SymbolicExpression> result_poly_coeffs;
        // 注意：由于包含负次幂，这里不能直接用 symbolic_polynomial_coefficients_from_simplified
        // 但 RDESolution 的 polynomial_part 设计可能只支持正次幂。
        // 我们暂时返回 true 并记录 value，或者调整 RDESolution
        solution->polynomial_part = SymbolicPolynomial::from_expression(result_expr, ext.t_name);
        return true;
    }
