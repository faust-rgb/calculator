...
- `mymath_float128.h` - High-precision math support
...
Parsing is handled by the `UnifiedParser` in `src/parser/unified_expression_parser.cpp`.
...
Implemented in `src/parser/exact_evaluator.cpp`.
...
Implementation:
- `src/parser/symbolic_render_parser.cpp`
...
The script engine in `src/execution/script_runtime.cpp` uses several optimizations:
...
In `src/parser/expression_compiler.cpp`, loop bodies are compiled once to AST
...
2. `ARCHITECTURE.md`
3. `test/suites/`
4. `src/core/calculator_core.cpp`
5. `src/execution/script_runtime.cpp`
6. `src/parser/unified_expression_parser.cpp`
