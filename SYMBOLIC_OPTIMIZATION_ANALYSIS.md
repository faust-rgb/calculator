...
本轮进一步深化了符号引擎的基础能力，并大幅提升了大规模计算下的稳定性，同时增强了 CLI 交互体验：

- **已完成：精确常数系统 (Phase 3 提前落地)**。引入 `kPi` 和 `kE` 专用节点类型，确保 $\pi$ 和 $e$ 在简化过程中不被过早折叠为浮点数。
- **已完成：规范化统一**。在 `simplify()` 中将 `e ^ x` 自动规范化为 `exp(x)`，统一了指数项的内部表示，使积分、求导和变换引擎能复用同一套模式匹配规则。
- **已完成：CSE (公共子表达式消除) 基础**。实现 `SymbolicExpression::common_subexpressions()`，支持对表达式树中重复出现的非平凡子树进行计数和提取。
- **已完成：节点驻留增量清理 (性能)**。将 `intern_node` 的全量扫描改为增量 LRU 淘汰（单次扫描限制为 32 项），消除了复杂符号运算中的 $O(N)$ 性能抖动。
- **已完成：CLI 交互体验增强 (非符号类)**。实现 REPL 的光标左右移动、Home/End 以及快捷键编辑，支持更复杂的长表达式输入与修改。
...
本轮解决了由于节点池满导致的高阶性能抖动问题，并为更深度的 CSE 提供了支持：

- 已完成：节点 interning 改为 **增量 LRU 淘汰**，确保了单次节点构造的 $O(1)$ 摊销成本。
- 已完成：公共子表达式 (CSE) 的**显式计数与提取接口**，为后续自动重写 Pass 奠定了基础。
- 已完成：通过 `e ^ x` 的规范化简化，降低了模式匹配逻辑的复杂度和冗余度。
