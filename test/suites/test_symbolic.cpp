...
int run_symbolic_tests(int& passed, int& failed) {
    ...
    const std::vector<ErrorCase> matrix_error_cases = {
        ...
        {"null(3)"},
        {"least_squares(mat(2, 1, 1, 1), mat(3, 2, 1, 2, 3, 4, 5, 6))"},
        ...
        {"solve(mat(2, 2, 1, 2, 3, 4), mat(3, 2, 1, 2, 3, 4, 5, 6))"},
    ...
    const std::vector<DisplayCase> command_display_cases = {
        ...
        {"laplace(a, t, s)", false, "a / s"},
        {"laplace(a * t^2, t, s)", false, "2 * a / (s * s * s)"},
        {"laplace(exp(a * t), t, s)", false, "1 / (-a + s)"},
        {"laplace(sin(a * t), t, s)", false, "a / (a ^ 2 + s ^ 2)"},
        {"laplace(cos(a * t), t, s)", false, "s / (a ^ 2 + s ^ 2)"},
        {"laplace(sinh(a * t), t, s)", false, "a / (-(a ^ 2) + s ^ 2)"},
        {"laplace(cosh(a * t), t, s)", false, "s / (-(a ^ 2) + s ^ 2)"},
        {"laplace(sqrt(t), t, s)", false, "1/2 * sqrt(pi) / s ^ (3/2)"},
        {"ilaplace(1 / s)", false, "step(t)"},
        {"ilaplace(1 / (s + 2), s, t)", false, "exp(-2 * t) * step(t)"},
        {"ilaplace(3 / (2 * s + 4), s, t)", false, "3/2 * exp(-2 * t) * step(t)"},
        {"ilaplace(1 / (s - a)^2, s, t)", false, "exp(a * t) * step(t) * t"},
        {"ilaplace(s / ((s - 1)^2 * (s - 2)), s, t)", false,
         "step(t) * (exp(t) * (-t - 2) + 2 * exp(2 * t))"},
        {"ilaplace(1 / ((s - a) * (s - b)), s, t)", false,
         "step(t) * (1 / (-a + b) * exp(b * t) + 1 / (-b + a) * exp(a * t))"},
        {"ilaplace((c * s + d) / ((s - a) * (s - b)), s, t)", false,
         "step(t) * ((a * c + d) / (-b + a) * exp(a * t) + (b * c + d) / (-a + b) * exp(b * t))"},
        {"ilaplace(exp(-a * s) / (s - b), s, t)", false,
         "exp(b * (-a + t)) * step(-a + t)"},
        {"ilaplace(exp(-2 * s) / (s - 1), s, t)", false,
         "0.135335283236 * exp(t) * step(t - 2)"},
        {"laplace(step(t - a), t, s)", false, "exp(-(a * s)) / s"},
        {"laplace(delta(t - a), t, s)", false, "exp(-(a * s))"},
