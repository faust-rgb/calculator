...
int run_core_logic_tests(int& passed, int& failed) {
    std::cout << "DEBUG: Running variable tests..." << std::endl;
    // 运行变量赋值与精确模式测试
    run_logic_variable_tests(passed, failed);

    std::cout << "DEBUG: Running symbolic help tests..." << std::endl;
    // 运行符号常量与帮助系统测试
    run_logic_symbolic_help_tests(passed, failed);

    std::cout << "DEBUG: Running varmanage base tests..." << std::endl;
    // 运行变量管理与进制转换测试
    run_logic_varmanage_base_tests(passed, failed);

    std::cout << "DEBUG: Running poly function tests..." << std::endl;
    // 运行自定义函数与多项式运算测试
    run_logic_poly_function_tests(passed, failed);

    std::cout << "DEBUG: Running calculus tests..." << std::endl;
    // 运行符号微积分测试
    run_logic_calculus_tests(passed, failed);

    std::cout << "DEBUG: Running multivar tests..." << std::endl;
    // 运行多元分析测试
    run_logic_multivar_tests(passed, failed);

    std::cout << "DEBUG: Running integral ODE tests..." << std::endl;
    // 运行多重积分与ODE测试
    run_logic_integral_ode_tests(passed, failed);

    std::cout << "DEBUG: Running limit root tests..." << std::endl;
    // 运行极限与求根测试
    run_logic_limit_root_tests(passed, failed);

    std::cout << "DEBUG: Running extrema func tests..." << std::endl;
    // 运行极值与自定义函数管理测试
